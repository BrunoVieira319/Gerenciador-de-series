...
class App extends Component {
  ...
  componentDidMount = () => {
    fetch('http://localhost:3009/list', {
      method: 'GET',
      headers: {
  ...
  editarSerie = (titulo) => {
    let novaList = this.state.list
    let index = novaList.findIndex((serie) => serie.titulo == titulo)
    let serieAlvo = novaList.find((serie) => serie.titulo == titulo)
  ...
  enviarSerieEditada = () => {
    let serieInfo = { 
      ...
      totalEpis: this.state.totalEpis,
      status: this.state.status,
      meta: this.state.serieAlvo.meta,
      $loki: this.state.serieAlvo.$loki
    };
  ...
  excluirSerie = (titulo) => {
    fetch(`http://localhost:3009/list/${titulo}`, {
      method: 'DELETE'
    }).then((response) => {
      if(response.status == 200) {
        let novaList = this.state.list.filter((serie) => serie.titulo != titulo);
